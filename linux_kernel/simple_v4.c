...
struct simple_data {
	...
	sector_t batch_sectors; //用于记录当前进程总共完成了多少请求
...
//	printk(KERN_INFO "idle, %u", (unsigned int)sd->batch_sectors);
...
static void simple_completed_request(struct request_queue *q, struct request *rq)
{
	...
	if (sd->io_context == RQ_IOC(rq) && sd->io_context) {
		if (sd->batch_sectors > 0) {
			// TODO
            //printk("sectors: %u\n", (unsigned int)sd->batch_sectors);
		}
...
static void simple_move_to_dispatch(struct simple_data *sd, struct request *rq)
{
	...
	if (sd->io_context != ioc) {
		...
		sd->batch_sectors += blk_rq_sectors(rq);
	}
